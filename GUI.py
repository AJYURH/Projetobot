...
def updater():
    ...
    while(1):
        ...
        try:
            ...
            '''
            with open ("test_data.cvs", "a") as f:
                writer = cvs.writer(f, delimiter = ',')
                writer.writerow([time.time(), decoded_bytes])
                if receivingStatusF == False and receivingPositionF == False and receivingSpeedF == False:

                    if(decoded_bytes == 'status'):
                        receivingStatusF = True
                        while(receivingStatusF == True):                        
                            try:
                                ser_bytes = ser.readline()
                    elif(decoded_bytes == 'position'):
                        receivingPositionF = True
                    elif(decoded_bytes == 'speed'):
                        receivingSpeedF = True
                    else:
                        print('invalid value')
                        break
                else:
                    print('invalid supervisor condition')
            '''                
        except:
            print('No Serial')
...
def openManual():
    ...
    def input():
        ...
        print('value a: %s \nvalue b: %s \nvalue c: %s' % (recievedSpeedX, recievedSpeedY, recievedSpeedΘ))
    ...
    iSpeedΘ = tkinter.Label (manualW, anchor = 'w', text = "Speed Θ", padx = 2)
    ...
    iSpeedΘ.grid(row = 4, column = 1, pady = 2, padx = 3, sticky = 'W')
